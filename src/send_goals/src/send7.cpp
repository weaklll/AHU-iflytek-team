...
void go_front(MoveBaseClient& ac)
{
    ...
    else if(min_front_dis<0.7)
    {
        ...
        goal3.target_pose.pose.orientation.w = 0.707;
...
bool getRobotPosition(tf::TransformListener& listener, double& x, double& y,double& z,double& w,double& yaw) 
{
                        ...
                        if (getRobotPosition(listener, x, y,z,w,yaw))
                         {
                            ...
                            // printf("此时的坐标(%f,%f)\n",x,y);
                            if((abs(x-1.94)<0.5)&&(abs(y+0.5)<0.12))    //终点的实际方位大概在(1.94,-0.15)
                            {
