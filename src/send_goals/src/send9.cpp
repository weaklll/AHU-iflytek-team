            ...
            switch (i)
            {
                ...
                case 6: //到终点，但不朝向巡线方向
                    ...
                    while (1) 
                    {
                        ...
                        //方案一，通过雷达左侧信息判断是否到达终点前一小段距离
                        if (getRobotPosition(listener, x, y,yaw))
                         {
                            if(x>1.7)
                            {
                                bool on_front_of_final = false;   //用于检测是否到了终点前一小段距离的标志位
                                ros::param::set("/detect_whether_front_of_final", true);
                                ros::param::get("/on_front_of_final", on_front_of_final);
                                while(1)
                                {
                                    printf("x=%f\n",x);
                                    ros::param::get("/on_front_of_final", on_front_of_final);
                                    if(on_front_of_final)
                                    {
                                        break;
                                    }
                                }
                        ...
                        // //方案二，通过巡线判断是否到达终点前一小段距离
                        //     bool on_front_of_final_by_visual=false; 
                        //     while(1)
                        //     {
                        //         ros::param::get("/on_front_of_final_by_visual", on_front_of_final_by_visual);
                        //         if(on_front_of_final_by_visual)
                        //         {
                        //             break;
                        //         }
                        //     }
