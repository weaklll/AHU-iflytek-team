...
// void turn_left(double jiaodu)
// {

//     ros::Publisher cmd_vel_pub = ros::NodeHandle().advertise<geometry_msgs::Twist>("/cmd_vel", 10);

//     double angular_speed = 2; // 设定角速度为2rad/s
//     double angle = DEG2RAD(jiaodu); // angle为弧度，这样为旋转90度。

//     double duration = angle / angular_speed;    // 计算旋转所需时间


//     geometry_msgs::Twist vel_msg;   // 创建一个Twist消息
...
void find(MoveBaseClient& ac)
{
    ...
    if(if_get)
    {
        ...
        double x_true_l = (dist_left)*cos(x_rad+0.006912195*6);//0.00747998是雷达信息角度差（弧度）=（360/840）*PI/180
        double y_true_l = (dist_left)*sin(x_rad+0.006912195*6);

        double x_true_r = (dist_right)*cos(x_rad-0.006912195*6);
        double y_true_r = (dist_right)*sin(x_rad-0.006912195*6);
        ...
        // double x_true = (dist-0.3)*cos(x_rad);
        // double y_true = (dist-0.3)*sin(x_rad);
        double x_true = dist*cos(x_rad)-0.25*sin(PI-act_k);
        double y_true = dist*sin(x_rad)-0.25*cos(PI-act_k);
        ...
        x_true = dist*cos(x_rad)-0.25*sin(PI-act_k);
        y_true = dist*sin(x_rad)-0.25*cos(PI-act_k);
        printf("**************************\n");
        printf("我将要前往的点(%f , %f)",x_true,y_true);
        printf("**************************\n");
        // if(k>0.11&&x_the>25)
        // {
        //     yaw_k=90-yaw_k;
                        ...
                        if (getRobotPosition(listener, x, y))
                         {
                            ...
                            if((abs(x-1.94)<0.2)&&(abs(y-0.050)<0.1))
                            {
