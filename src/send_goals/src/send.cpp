        ...
        if (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
        {
            ...
            if(i==2)
            {
                ...
                while(1)
                {
                    ...
                    ros::param::get("/dist_left",dist_left);//中点偏左
                    ros::param::get("/dist_right",dist_right);//中点偏右
                    if(dist>0&&dist_left>0&&dist_right>0)
                    {
                        break;
                    }
                }
                std::cout<<"该角度上的物品距离为:"<<dist<<std::endl;
                double x_true = (dist-0.6)*cos(x_rad);
                double y_true = -(dist-0.6)*sin(x_rad);
                ...
                double k = (y_true_l-y_true_r)/(x_true_l-x_true_r);//两直线斜率
