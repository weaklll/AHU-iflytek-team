...
void find(MoveBaseClient& ac)
{
    ...
    double x_tan;
    x_tan = (320-x_mid)*6.95/2800;

    // if(250<x_mid<390)
    // {
    //     x_tan = (320-x_mid)*6.86/2800;//算出成像方的角度-----6.86为算出来的像元大小
    //     std::cout<<"near center"<<endl;
    ...
    double x_true = (dist-0.4)*cos(x_rad);
    double y_true = (dist-0.4)*sin(x_rad);//加负号是因为在图像中，将其镜像处理了
    // double x_true = dist*cos(x_rad)-0.05;
    // double y_true = dist*sin(x_rad)-0.05;
