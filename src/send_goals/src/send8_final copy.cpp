        ...
        if (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
        {
            ...
            switch (i)
            {
                ...
                case 6: //到终点，但不朝向巡线方向
                    ...
                    while(1)
                    {
                        if(getRobotPosition(listener, x, y,yaw))
                        {
                            // printf("yaw=%f",yaw);
                            // printf("yaw=%f",yaw);
                            // printf("yaw=%f",yaw);
                    ...
                    {
                        //发点
                        //-0.030 0.025 -0.714 0.701
                        ROS_ERROR("通过发点转的");
                        move_base_msgs::MoveBaseGoal goal_xunxian;

                        goal_xunxian.target_pose.header.frame_id = "map";
                        goal_xunxian.target_pose.pose.position.x=-0.030;
                        goal_xunxian.target_pose.pose.position.y = 0.025;
                        goal_xunxian.target_pose.pose.orientation.z = -0.714;
                        goal_xunxian.target_pose.pose.orientation.w = 0.707;

                        ac.sendGoal(goal_xunxian);
                        ROS_INFO("Sending Goal left_up");
                        ac.waitForResult();
                        if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
                        {
                    ...
                    while (1) 
                    {
                        ...
                        //     // if((fabs(x-1.94)<0.5)&&(fabs(y+0.1)<0.2))
                        //     if((fabs(x-1.94)<0.5)&&(y > -0.4) && (y < -0.2))
                        //     {
                        //         // move_base_msgs::MoveBaseGoal goal_final;
                        //         // //1.944 0.012 0.711 0.703 
                        //         // goal_final.target_pose.header.frame_id = "map";
                        //         // goal_final.target_pose.header.stamp = ros::Time::now();
                        //         // goal_final.target_pose.pose.position.x = 1.944;
                        //         // goal_final.target_pose.pose.position.y = -0.1;//0.099
                        //         // goal_final.target_pose.pose.orientation.z = 0.711;
                        //         // goal_final.target_pose.pose.orientation.w = 0.703;

                        //         // ac.sendGoal(goal_final);
                        //         sleep(2);
                        //         ROS_INFO("go to final!!!!!!!!!!");
                        //         // ac.waitForResult();
                        //         // if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
                        //         // {
                        //         //     ROS_INFO("到达终点");
...
void odomCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    ros::param::get("/if_follow_line", final);
    if(final==1)
    {
        // 打印获取到的信息
        ...
        if((fabs(x-1.94)<0.5) && y>=-0.6 && y<-0.4)
        {
...
void timerCallback(const ros::TimerEvent&)
{
    ...
    ros::param::get("/if_follow_line", final);
    if (cmd_vel_received && (current_time - last_cmd_vel_time > timeout)&&final==1)
    {
...
int main(int argc,char*argv[])
 {
    ...
    //巡线初始化
    // 指定目标点文件的绝对路径
    string goals_file = "/home/iflytek/move/src/send_goals/src/goals/goals8.txt";
    // 指定启动文件的绝对路径
    ...
    ros::Subscriber teb_feedback_sub = nh.subscribe("/move_base/TebLocalPlannerROS/teb_feedback", 10, tebFeedbackCallback);
    // 设置定时器，每秒检查一次
