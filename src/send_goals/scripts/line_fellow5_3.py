...
"""
特别注意：
不同车的differ值，车距不同，需要手动更新
决赛代码：适用于直角弯和简单弯道，无矫正切线
...
class LaneLines:
    ...
    def fit_poly(self, img, lane=2):
        ...
        # 确定边线
        left_x, left_y,left_ang,left_ang_sum,left_zero,left_line,left_line_posi,left_ang_posi = self.find_lane_pixels(img, lane=0)
        right_x, right_y,right_ang,right_ang_sum,right_zero,right_line,right_line_posi,right_ang_posi = self.find_lane_pixels(img, lane=1)
    ...
    def calculate(self, lane=1):
        ...
        if lane_change :
            ...
            # print(f"R-gradiant = {gradiant}")
        
        else :
            gradiant = self.left_fit[0]
            # print(f"L-gradiant = {gradiant}")
